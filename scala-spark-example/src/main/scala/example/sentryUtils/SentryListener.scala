...
class SentryListener extends SparkListener with Logging {
    ...
    sentry
      ...
      .recordBreadcrumb(
        new BreadcrumbBuilder()
          ...
          .withData("time", epochMilliToDateString(time))
          .build()
      );
  }
  ...
  override def onTaskEnd(taskEnd: SparkListenerTaskEnd) {
    ...
    reason match {
      case _: TaskFailedReason => TaskEndParser.parseTaskEndReason(reason.asInstanceOf[TaskFailedReason])
      case _ =>
    }
  }
  ...
  object TaskEndParser {
    def parseTaskEndReason(reason: TaskFailedReason) {
      reason match {
        case _: ExceptionFailure => this.captureExceptionFailure(reason.asInstanceOf[ExceptionFailure])
        case _: ExecutorLostFailure => this.captureExecutorLostFailure(reason.asInstanceOf[ExecutorLostFailure])
        case _: FetchFailed => this.captureFetchFailed(reason.asInstanceOf[FetchFailed])
        case _: TaskCommitDenied => this.captureTaskCommitDenied(reason.asInstanceOf[TaskCommitDenied])
